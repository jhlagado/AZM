...
export function parseGlobalsBlock(
  ...
  if (storageHeader === 'var') {
    diag(diagnostics, modulePath, `Legacy "var ... end" storage blocks are removed; use labels with .db/.dw/.ds directives.`, {
      line: lineNo,
      column: 1,
    });
  } else {
    diag(
      diagnostics,
      modulePath,
      'Legacy "globals ... end" storage blocks are removed; use labels with .db/.dw/.ds directives.',
